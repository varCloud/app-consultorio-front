...
@Component({
  selector: 'app-mdl-registro-operador',
  templateUrl: './mdl-registro-operador.component.html',
  styleUrls: ['./mdl-registro-operador.component.scss']
})
export class MdlRegistroOperadorComponent implements OnInit {
  ...
  onActivarForm(){
    ...
    this.frmRegistro.contrasena.enable();
  }
  ...
  obtenerTiposUsuarios() {
    this.catalogoService.obtenerTiposUsuariosPorRol({}).subscribe((data: any) => {
      if (data.estatus == 200) {
        ...
        this.frmRegistro.idTipoUsuario.setValue(EnumTipoUsuario.Operador)
