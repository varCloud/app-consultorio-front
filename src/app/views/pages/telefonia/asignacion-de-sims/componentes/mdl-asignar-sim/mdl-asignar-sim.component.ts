...
@Component({
  selector: 'app-mdl-asignar-sim',
  templateUrl: './mdl-asignar-sim.component.html',
  styleUrls: ['./mdl-asignar-sim.component.scss']
})
export class MdlAsignarSimComponent implements OnInit {
  ...
  obtenerSims() {
    let postData = {
      ...
      idEstatusSim: this.idEstatusSim,
      idTipoUsuario: this.sesionService.sesion.idTipoUsuario,
      idEstatusAsignacionSim: this.idEstatusAsignacionSim
    }
    this.simService.consultarSims(postData).subscribe((data: any) => {
      if (data.estatus == 200) {
        this.lstSims = [];
