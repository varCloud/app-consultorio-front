...
@Component({
  ...
  styleUrls: ['./transacciones-diarias.component.scss']
})
export class TransaccionesDiariasComponent implements OnInit {
  ...
  getReporteVentas(){
    ...
    this.reporteriaService.getReporteVentas(postData).subscribe((data: any) => {
      if (data.status == 200) {
  ...
  updateFilter(value: any) {
    if (value === '') {
      ...
      const filtered = this.allData.filter((data: any) => {
        return data.oferta.toLowerCase().includes(value.toLowerCase()) ||
        data.celular.toLowerCase().includes(value.toLowerCase()) ||
        data.nombreCliente.toLowerCase().includes(value.toLowerCase()) ||
        data.canalIntegracion.toLowerCase().includes(value.toLowerCase()) ||
        data.monto == value ||
        data.folioAltan.toLowerCase().includes(value.toLowerCase()) ||
        data.idOfertaAltan.toLowerCase().includes(value.toLowerCase()) ||
        data.folioTransaccion.toLowerCase().includes(value.toLowerCase())
      });
